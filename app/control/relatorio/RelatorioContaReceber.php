...
class RelatorioContaReceber extends TPage
{
    ...
    function onGenerate()
    {
        try
        {
            ...
            // define the query
            $query = 'SELECT conta.descricao, conta_receber.valor, conta_receber.data_vencimento, conta_receber.valor_pago, pessoa.nome                            
            ...
            {
                $query .= " and conta_receber = {$this->data->conta_id}";
            }
            ...
            if ($rows)
            {
                ...
                if (!empty($table))
                {
                    ...
                    $table->setHeaderCallback( function($table) {
                        $table->addRow();
                        $table->addCell('Relatorio Dívida Unidade', 'center', 'header', 6);
                        // Pega data inicio e data fim imprimindo no relatório 
                        $table->addRow();
                        $table->addCell('Data Início: ' . $this->data->data_inicio . ' - Data Fim: ' . $this->data->data_fim, 'center','title',6);
                       
                        $table->addRow();
                        $table->addCell('Conta', 'center', 'title');
                        $table->addCell('Vencimento', 'center', 'title');
                        $table->addCell('Valor', 'right', 'title');
